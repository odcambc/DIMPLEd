...
class DimpleApp(QMainWindow):
    ...
    def select_input_file(self):
        ...
        # If output dir is not set, update it to the directory containing the input file.
        if not self.output_dir:
            self.output_dir = Path(self.input_file).parent
            self.output_dir_label.setText(f"Output directory: {self.output_dir}")
            self.output_area.append(f"Output directory set to {self.output_dir}")
    ...
    def run_dimple(self):
        # Set up the logging output file

        # Check if self.output_dir is defined first.
        if not self.output_dir:
            self.output_area.append("Output directory not selected.")
            return None
        ...
        # Run DIMPLE and catch exceptions
